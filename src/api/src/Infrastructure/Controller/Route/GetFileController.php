...
final class GetFileController extends AbstractController
{
     ...
     * @Route("/file/serve/{id}")
     */
    public function serveFile(string $id, Request $request): Response
    {
        ...
        if ($upstream['scheme'] === 'file' && empty($upstream['host'])) {
            $filepath = $this->envVarHelper->fetch(EnvVarHelper::ROOT_PATH) . $upstream['path'];
            ...
            $headers = [];
            if (isset($pathinfo['extension']) && isset(self::EXTENSION_MIMES[strtolower($pathinfo['extension'])]) && ! $request->query->has('download')) {
                $headers = [
