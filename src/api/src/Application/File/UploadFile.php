...
final class UploadFile
{
    ...
    public function upload(UploadedFileInterface $file, string $rootPath): FileDescriptor
    {
        ...
        if (! file_exists($rootPath . 'uploads/files')) {
            mkdir($rootPath . 'uploads/files', 0757, true);
        }
        $upstream = 'uploads/files/' . hash('sha256', (string) mt_rand()) . (isset($filename['extension']) ? '.' . $filename['extension'] : '');
