...
final class FinishProgram
{
     ...
     */
    public function finish(string $programId): Program
    {
        $program = $this->programRepository->mustFindOneById($programId);
        $now = $this->getCurrentTime();

        if ($now < $this->convertTime($program->getDateStart())) {
            throw new InvalidDateValue('Program is yet to start');
        }

        $program->setStatus(ProgramStatusEnum::FINISHED);
