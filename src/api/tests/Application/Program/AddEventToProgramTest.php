...
class AddEventToProgramTest extends ApplicationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $eventDate = $this->faker->dateTimeBetween('now', '+10 days');
        for ($i = 0; $i < self::NUM_EVENTS; $i++) {
            $eventDate = $eventDate->modify('+1 day');
            ...
            $event->setDateEventEnd(DateTimeImmutable::createFromFormat(DateTimeImmutable::ISO8601, $eventDate->modify('+1 hour')->format('c')));
    ...
    public function testAddEventToProgram(): void
    {
        ...
        $this->assertEquals($this->events[self::NUM_EVENTS - 1]->getDateEventEnd()->getTimestamp(), $this->program->getDateEnd()->getTimestamp());
    }
