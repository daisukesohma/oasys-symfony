...
- Logging : Here we have ModelLogger which a “trait” that allows to reuse the code of setting creation information such as user and date. In order to use the trait you just need to add “use ModelLogger” in your class. You must as well implement a save function in the dao which calls **"$this->log($object);"** and implement the **"LoggableModel"** on the Model.
