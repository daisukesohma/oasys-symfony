<template>
  <div class="wrapper wrapper--dash d-flex">
          ...
          <template v-for="(link,i) in sidebarLinks">
            ...
            >
              <router-link
                ...
                class="nav-link"
                :class="{'active': link.activeOn ? link.activeOn.includes($route.name) : modeIs(link.key)}"
              >
                ...
                <img
                  class="sb-img"
                  :src="require(`@/assets/img/${link.icon}${(link.activeOn ? link.activeOn.includes($route.name) : modeIs(link.key)) ?'-active':''}.svg`)"
                />
                </el-tooltip>
              </router-link>
            </li>
          </template>
          <li class="nav-item main-action" v-if="hasMainAction">
      ...
      <div class="nav-wrap nav-wrap--two">
        <ul class="nav bottom-nav">
          <li class="nav-item avatar">
            <span class="nav-link" :class="{'active': $route.name === 'Me'}">
              <el-tooltip effect="light" content="Mon compte" placement="right">
                <button class="btn btn-avatar" :class="{'active': modeIs('users')}" @click="goToMyProfile()">
                  <img
                    class="avatar-img"
                    v-if="profilePicture"
                    :src="profilePicture"
    ...
    <div class="wrapper-content flex-grow-1 d-flex flex-column" :class="{'bg-big candidate-home': candidateHome}">
      <div class="wrapper-content--header container-fluid">
        <slot name="header">
          <div class="d-flex align-items-center">
            <span v-if="isCoach" class="sub-txt text-white header-title">
              Bonjour
              <span class="main-txt font-weight-light">{{me.firstName}}</span>
            </span>
            <span v-else class="sub-txt text-white">
              {{article}}
  ...
  computed: {
    sidebarLinks() {
      return [
        {
          key: "candidateHome",
          activeOn: ['CandidateHome', 'CandidateFaq'],
          icon: "icon-home",
          routeName: "CandidateHome",
          label: "Accueil",
          enabled: this.isCandidate,
        },
        {
          key: "candidateDashboard",
          activeOn: ['CandidateDashboard'],
          icon: "icon-coaching",
        ...
        {
          key: "users",
          activeOn: ['CoachDashboard', 'ViewCandidate'],
          icon: "coache",
          routeName: "CoachDashboard",
          label: "Tableau de bord",
          enabled: this.isCoach
        },
        {
          key: "folders",
          icon: "folders",
    ...
    hasMainAction(){
      return this.$slots.mainaction && this.$slots.mainaction.length > 0
    }
  },
  methods: {
    goToMyProfile() {
    ...
    modeIs(mode) {
      return typeof mode.includes !== 'undefined' ? mode.includes(this.mode.toLowerCase()) : this.mode.toLowerCase() === mode.toLowerCase();
    },
    logout() {
      this.$apollo.mutate({
          mutation: LOGOUT
