<template>
  <dash-wrap :hideTabs="true">
    <template v-slot:mainaction>
      <router-link class="btn nav-link" :to="{name:'UsersForm', params: {id: $route.params.id}}">
    ...
    <div class="profile-wrap">
      ...
      <template v-else>
        <div class="form-btn-top">
          ...
          <button type="button" @click="openTransferUserModal" class="btn btn-secondary px-5 ml-2">
          ...
          <div class="profile-container">
            ...
            <div class="form-card profile-card">
              ...
              <div class="row profile-details profile-details--two">
                <div :class="isCandidat ? 'col-md-5 section-1 offset-md-1' : 'col-md-12'" class="col-sm-12 pt-0 mt-0 offset-0">
                ...
                <div v-if="isCandidat" class="col-sm-12 col-md-5 pt-0 mt-0 section-2">
          ...
          <div class="row mt-2">
            <div class="col-md-12">
              ...
              <list-programs ref="list"
                             ...
                             @expand="program => toggleProgramEvents(program)"
                             @collapse="program => toggleProgramEvents(program)"
              />
            </div>
          </div>
        </div>
    ...
    <transfer-user-modal v-if="transferUserModal.show"
                         :value="user.coachSpeciality ? user.coachSpeciality.id : ''"
                         @close="closeTransferUserModal"
                         @confirm="$event => transferUser($event)" />
  </dash-wrap>
</template>
...
import {TRANSFER_CANDIDATE} from "@/graphql/user/transfer-candidate-mutation";
...
export default {
  ...
  data() {
    return {
      ...
      header: [
          ...
          {key: 'actions', label: 'Actions', actions: [ 'expand', 'collapse','edit']},
      ],
      ...
      showProgramEvents: null,
    };
  },
  computed: {
    isCandidat() {
        return 'candidate' === this.user.type.id;
    },
    listPrograms () {
        return this.user.programsByProgramsUsers ? this.user.programsByProgramsUsers
            .map(item => ({
                ...
                eventCount: item.eventsOrderedByDate.count,
                events: item.eventsOrderedByDate.items.map(event => ({
                  id: event.id,
                  programId: item.id,
                  name: event.name,
                  coach: event.organizer.firstName + ' ' + event.organizer.lastName,
                  status: EVENT_STATUS[event.status],
                  dateEvent: event.dateEvent ? localMoment(event.dateEvent).format('DD/MM/YYYY, HH:mm') + ' à ' + localMoment(event.dateEventEnd).format('HH:mm') : null,
                })),
                _actions: {
                  expand: item.eventsOrderedByDate.items.length > 0 && item.id !== this.showProgramEvents,
                  collapse: item.id === this.showProgramEvents,
                },
            })) : []
    },
  },
  ...
  methods: {
    ...
    dateFormat (date) {
      return date ? localMoment(date).format('DD/MM/YYYY') : '';
    },
    getLinkedinURL(linkedin) {
      return linkedin.indexOf('http')>-1 ? (linkedin) : ('https://' + linkedin)
    },
    openTransferUserModal() {
      this.transferUserModal.show = true;
    },
    closeTransferUserModal() {
      this.transferUserModal.show = false;
    },
    toggleProgramEvents(program) {
      this.showProgramEvents = this.showProgramEvents === program.id ? null : program.id;
    },
    transferUser(coachSpeciality) {
      this.$apollo.mutate({
      ...
      izitoast.success({
        ...
        position: "topRight",
        title: "Succès",
        message: "L'utilisateur a été transféré",
      });
    }
  },
};
