    ...
    <div class="container text-left user-form my-5">
      <div class="user-change-icon profile-picture" v-if="!loading">
        ...
        <div class="edit-icon">
          ...
          <label for="user-logo" class="m-1 p-0">
      ...
      <div class="content flex-grow-1">
        ...
        <form v-if="!loading">
            ...
            <div class="form-group col-md-6">
              <div class="custom-control custom-switch" v-if="!loading">
                <input
                  ...
                  v-model="user.status"
                />
                <label class="custom-control-label" for="status">{{user.status ? 'Actif' : 'Inactif'}}</label>
              </div>
            </div>
          </div>
          <fieldset>
            ...
            <div class="panel panel-default">
              <div class="panel-body">
                <div class="form-row">
                  ...
                  <div class="col-md-2 offset-md-1 form-group radio" v-if="hasType">
                    <label>Civilité</label>
                    <div class="radio-items mt-2">
                      <div class="radio-item">
                        <input class="form-check-input" name="civility" v-validate="'required'" type="radio" value="m" v-model="user.civility" id="civility_m" />
                        <label class="form-check-label" for="civility_m">M.</label>
                      </div>
                      <div class="radio-item ml-1">
                        <input class="form-check-input" name="civility" v-validate="'required'" type="radio" value="mme" v-model="user.civility" id="civility_mme" />
                        <label class="form-check-label" for="civility_mme">Mme</label>
                      </div>
                    </div>
                    <div v-if="errors.has('civility')" class="invalid-feedback">
                      <p>{{ " Le champ Civilité est obligatoire " }}</p>
                    </div>
                  </div>
                  <div class="col-md-3 pl-4 form-group" v-if="isCandidate">
                ...
                <div class="form-row" v-if="isCandidate">
                  ...
                  <div v-if="user.typeId === CANDIDATE" class="form-group col-md-5 offset-md-1">
                    ...
                    <input
                      ...
                      v-validate="'required'"
                      :disabled="isEdit"
                      v-model="user.programType"
                      :class="{'is-invalid': errors.has('programType')}"/>
                    <div
                      v-if="errors.has('programType')"
                      class="invalid-feedback d-block">
                      <p>{{ " Le champ Type prestation est obligatoire " }}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <fieldset v-if="hasType">
            ...
            <div class="panel panel-default">
              <div class="panel-body">
                ...
                <div class="form-row" v-if="isCoach">
                  <div class="col-md-5">
                    <div class="form-group select">
                      ...
                      <div v-if="errors.has('coachSpeciality')" class="invalid-feedback d-block">
                        <p>Le champ Spécialité est obligatoire</p>
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <villes-france-autocomplete
                                  :initial-query="user.postCode"
                                  :value="user.postCode"
                                  name="postCode"
                                  @query="setPostCode"
                                  @item-selected="updatePostCode"
                ...
                <div class="form-row" v-if="isCandidate">
                  <div class="col-md-5">
                    <div class="form-group">
                      ...
                      <input
                              v-model="user.annualCompensation"
                  ...
                  <div class="col-md-5 offset-md-1">
                    <div class="form-group select">
                      ...
                      <select v-model="user.professionalCategory" name="professionalCategory" class="form-control" v-validate="'required'">
                        <option value disabled>Sélectionner une Catégorie</option>
                        <option v-for="category in professionalCategories" :value="category.id" :key="category.id">
                          {{ category.label }}
                        </option>
                      </select>
                      <div v-if="errors.has('professionalCategory')" class="invalid-feedback d-block">
                ...
                <div class="form-row" v-if="isCandidate">
                  <div class="col-md-5">
                    <div class="form-group">
                      ...
                      <input
                              v-model="user.nLastName"
                              v-validate="'required'"
                              id="nLastName"
                              name="nLastName"
                              class="form-control"
                              :class="{ 'is-invalid': errors.has('nLastName') }"
                              type="text"
                              placeholder="Nom du N+1"
                      />
                      <div v-if="errors.has('nLastName')" class="invalid-feedback">
                        <p>{{ " Le champ Nom du N+1 est obligatoire " }}</p>
                      </div>
                    </div>
                  </div>
                  <div class="col-md-5 offset-md-1">
                    <div class="form-group">
                      ...
                      <input  v-model="user.nFirstName"
                              v-validate="'required'"
                              id="nFirstName"
                              name="nFirstName"
                              class="form-control"
                              :class="{ 'is-invalid': errors.has('nFirstName') }"
                              type="text"
                              placeholder="Prénom du N+1" />
                      <div v-if="errors.has('nFirstName')" class="invalid-feedback">
                        <p>{{ " Le champ Prénom du N+1 est obligatoire " }}</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="form-row" v-if="isCandidate">
                  <div class="col-md-5">
                    <div class="form-group">
                      ...
                      <input
                              ...
                              id="nEmail"
                              name="nEmail"
                              :class="{ 'is-invalid': errors.has('nEmail') }"
                              type="email"
                              class="form-control"
                              placeholder="you@example.com"
                      />
                      <div v-if="errors.has('nEmail')" class="invalid-feedback">
                  ...
                  <div class="col-md-5 offset-md-1">
                    <div class="form-group">
                      ...
                      <input
                              :class="{ 'is-invalid': errors.has('nPhone') }"
                              class="form-control"
                              id="nPhone"
                              name="nPhone"
                      ...
                      <div v-if="errors.has('nPhone') && errors.firstByRule('nPhone', 'required')"
                              class="invalid-feedback">
                        <p>{{ " Le champ Téléphone du N+1 est obligatoire " }}</p>
                      </div>
                      <div v-else-if="errors.has('nPhone')" class="invalid-feedback">
                        <p>{{ " Le numéro de téléphone du N+1 est dans un format invalide " }}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <fieldset v-if="isCandidate || isCoach">
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <villes-france-autocomplete
                              ...
                              :value="user.userCodePostal"
                              name="userCodePostal"
                              @query="setUserPostCode"
          ...
          <fieldset v-if="hasType && user.typeId === 'coach'">
            <legend>CV</legend>
            <div class="panel panel-default">
              <div class="panel-body">
                <upload-file-drop label="Cliquez ici pour ajouter votre fichier"
                                  :value="user.cvFile"
                                  :cv-formats="true"
                                  @select-file="saveCVFile"
                                  @remove-file="removeCVFile" />
              </div>
            </div>
          </fieldset>
          <fieldset v-if="hasType">
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.$validator.validate().then(async valid => {
        ...
        let cvFileId = this.user.cvFileId;
        if (this.user.cvFileSelected) {
          let response = await this.$apollo.mutate({
            mutation: UPLOAD_FILE,
            variables: {
              file: this.user.cvFileSelected
            }
          });
          cvFileId = response.data.uploadFile.id;
        }
          ...
          .mutate({
            ...
            variables: {
              ...
              birthDate: this.user.birthDate
                      ? moment(this.user.birthDate, "D/M/YYYY").toISOString()
                      : null
            },
            update(proxy) {
              if (isNew) {
    ...
    saveCVFile(file) {
      this.user.cvFileSelected = file;
    },
    removeCVFile() {
      this.user.cvFileSelected = null;
    },
    setPostCode(code) {
      this.user.postCode = code;
    },
    updatePostCode(ville) {
      this.user.ville = ville.commonName;
      this.user.department = ville.departmentName;
      this.user.postCode = ville.codePostal;
    },
        ...
        update: data => {
          return {
            ...
            profilePictureId: data.userById.profilePicture
              ...
              : "",
            cvFileId: data.userById.cvFile
              ? data.userById.cvFile.id
              : "",
            professionalCategory: data.userById.professionalCategory ? data.userById.professionalCategory.id : null,
            ...
            coachSpeciality: data.userById.coachSpeciality ? data.userById.coachSpeciality.id : "",
          };
        },
        loadingKey: "loading"
      });
